...
public class PacketTransformer {
	...
	private static <B extends ByteBuf, T> T newBuffer(StreamCodec<B, T> codec, BufferFiller<B> filler) {
		B buffer = (B) new RegistryFriendlyByteBuf(Unpooled.buffer(), TransformerRequests.server.registryAccess());
		filler.fill(buffer);
	...
	private static Node wrapNode(ServerPlayer player, Node node) {
		...
		RegistryFriendlyByteBuf nodeBuffer = new RegistryFriendlyByteBuf(Unpooled.buffer(), player.getServer().registryAccess());
